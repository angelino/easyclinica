...
public class HealthCarePlanController extends BaseController {
	...
	@Get
	@Path("/convenios/novo")
	public void newForm(HealthCarePlan healthCarePlan) {
		HealthCarePlan emptyPlan = healthCarePlan == null ? HealthCarePlan.empty() : healthCarePlan;
		include(emptyPlan);
	...
	public void update(final HealthCarePlan healthCarePlan) {
		translator.translate(healthCarePlanValidator.validate(healthCarePlan));
		validator.onErrorUse(Results.logic())
				.forwardTo(HealthCarePlanController.class)
				.edit(healthCarePlan.getId());
